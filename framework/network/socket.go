...
func (s *Socket) HandleChannelStreamDatagram() {
	...
	for {
		...
		s.GetFecDecodeInfo(data) //持续接收数据
		...
		//	if err != nil {
		//		slog.Error("解包fec过程发生错误", slog.Any("channelId", packet.Header.ChannelId), slog.Any("err", err))
		//		continue
		//	}
		//}
	}
...
func (s *Socket) InitFecParam(channelId int) error {
	...
	if config.FecEnableLevel != FecDisabled {
		if config.Type == Audio { //音频大约一个数据包是640大小，我们直接塞进一个里面
			...
			packetizer.ParityShards = make([][]byte, config.ParityShards)
...
func (s *Socket) GetFecDecodeInfo(data []byte) *FecPacket {
	...
	case RtpHeader:
		if data[1] == AudioHeader || data[1] == AudioDynamicHeader { //音频数据包
			...
			packet.HeaderRaw = data[:packet.Header.HeaderSize]
	...
	case VideoHeader:
		if size <= VideoHeaderLength {
			slog.Debug("收到无效的video rtp数据包", slog.Int("len", size), slog.Any("data", data))
			return nil
		}
		if packet.Header.Type != 0 {
			slog.Debug("未支持的rtp 视频数据包")
			return nil
		}
		packet.HeaderRaw = data[:VideoHeaderLength]
	...
	case CustomFecHeader:
		...
		if packet.Header.Type != CustomMessageType {
		...
		packet.HeaderRaw = data[:FecPacketHeaderLength]
	...
	if !exists {
		depacketizer = NewDepacketizer(sc) //实时创建第2个ssrc之后的解包器
...
func (s *Socket) SendFecDatagram(channelId int, data []byte) (bool, error) {
	...
	case Audio: //音频模式特殊应用，他还不能有关键帧，如果超时丢弃了，数据包也是不要的，直接用下一个,使用的是标准的rtp数据包
		...
		packetizer := channel.Packetizers[0]
		packetizer.SharedShards[index] = data[RtpHeaderLength:dataSize]
		if data[0] != RtpHeader {
			slog.Debug("发送的数据包不是合格的rtp包,audio data")
		}
		_ = s.Conn.SendDatagram(data)     //发送处理好的数据
		if index == config.DataShards-1 { //生成fec
			encoder, err := packetizer.GetFecEncoder(config.DataShards, config.ParityShards)
			if err != nil {
				return false, err
			}
			...
			for i := config.DataShards; i < totalShards; i++ { //补发奇偶校验包
				if packetizer.SharedShards[i] == nil || cap(packetizer.SharedShards[i]) != parityShardSize {
					packetizer.SharedShards[i] = packetizer.ParityShards[i-config.DataShards][AudioHeaderLength:parityShardSize]
				}
				clear(packetizer.SharedShards[i]) //清除数据防止脏读
			}
			err = encoder.Encode(packetizer.SharedShards) //生成fec结果
			if err != nil {
				slog.Debug("打包fec过程发生错误", slog.Int("channelId", channelId),
					slog.Any("sequenceNumber", sequenceNumber), slog.Int("length", len(packetizer.SharedShards)),
			...
			for i := uint8(0); i < config.ParityShards; i++ { //补发奇偶校验包
				packetizer.ParityShards[i][0] = data[0]
				packetizer.ParityShards[i][1] = 127
				binary.BigEndian.PutUint16(packetizer.ParityShards[i][2:], sequenceNumber+uint16(i)+1)
				copy(packetizer.ParityShards[i][4:12], data[4:12])
				packetizer.ParityShards[i][12] = i //fecShardIndex
				packetizer.ParityShards[i][13] = 97
				binary.BigEndian.PutUint16(packetizer.ParityShards[i][14:], baseSequenceNumber)
				binary.BigEndian.PutUint32(packetizer.ParityShards[i][16:], baseTimestamp)
				copy(packetizer.ParityShards[i][20:24], data[8:12])
				_ = s.Conn.SendDatagram(packetizer.ParityShards[i][:parityShardSize]) //发送处理好的数据
			}
			//slog.Debug("执行音频fec打包发送", slog.Any("ParityShards", config.ParityShards))
		}
	case Video: //支持媒体流传输的特殊协议
		...
		packetizer, exists := channel.Packetizers[ssrc]
		if !exists {
			packetizer = NewPacketizer()
			channel.Packetizers[ssrc] = packetizer
		}
		packetizer.PacketIndex += 1
		...
		if parityShards < s.FecMinRequiredPackets && fecPercentage != 0 {
			...
			slog.Debug("打包fec过程因fec parity shard 太少，增加fec百分比", slog.Any("百分比", fecPercentage))
		}
		...
		for i := 0; i < int(dataShards); i++ {
			...
			buffer[i][16] = packetizer.PacketIndex             //将数据藏在 剩余协议位置里
		...
		if fecPercentage != 0 {
			...
			encoder, err := packetizer.GetFecEncoder(dataShards, parityShards)
			if err != nil {
				return false, err
			}
	...
	default: //数据分割模式
		packetizer, exists := channel.Packetizers[0]
		if !exists {
			packetizer = NewPacketizer()
			channel.Packetizers[0] = packetizer
		}
		packetizer.FrameIndex += 1
		...
		encoder, err := packetizer.GetFecEncoder(targetDataShards, targetParityShards)
		if err != nil {
			return false, err
		}
		...
		s.SafeWaitMillisecond(time.Millisecond)
		packetizer.GroupIndex++
		for index, shard := range shards {
			_ = s.sendFecShardDatagram(uint8(channelId), 0, 0, uint16(packetizer.FrameIndex), packetizer.FrameIndex,
				0, packetizer.GroupIndex, uint8(index), targetDataShards, targetParityShards, uint32(dataSize), shard)
		}
	}
	return true, nil
}

func (s *Socket) sendFecShardDatagramByHeader(header *FecPacketHeader, data []byte) error {
	return s.sendFecShardDatagram(header.ChannelId, header.Ssrc, header.Idr, header.SequenceNumber, header.FrameIndex,
		header.BlockIdx, header.GroupIdx, header.ShardIdx, header.DataShards, header.ParityShards, header.Total, data)
}

func (s *Socket) sendFecShardDatagram(channelId, ssrc, idr uint8, sequenceNumber uint16, frameIndex uint32,
	blockIndex, groupIndex, shardIndex, dataShards, parityShards uint8, total uint32, data []byte) error {
	slog.Debug("使用自定义fec发送消息", slog.Any("channelId", channelId), slog.Any("ssrc", ssrc), slog.Any("data", data), slog.Any("sequenceNumber", sequenceNumber))
	length := len(data)
	...
	buf[2] = channelId
	buf[3] = ssrc
	binary.BigEndian.PutUint32(buf[4:], frameIndex)
	binary.BigEndian.PutUint16(buf[8:], sequenceNumber)
	buf[10] = idr
	buf[11] = blockIndex
	buf[12] = groupIndex
	buf[13] = shardIndex
	...
	s.PacketPool.Put(bufPtr) // 函数结束归还 Pool
	slog.Debug("使用自定义fec发送消息成功！")
	return err
}
